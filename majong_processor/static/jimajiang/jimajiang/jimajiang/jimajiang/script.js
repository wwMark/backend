...
let Players = localStorage.getItem('Players') !== null ? localStoragePlayers : ['spring', 'summer', 'autumn', 'winter']; 
...
function recordScore(sv, roundtr){
  // Get sign 
  const sign = sv < 0 ? '-' : '+';
  //table method
  ...
  playertd.innerHTML = `${sign}${Math.abs(sv)}`;
...
function addScoreDOM(oneRoundScore) {
  ...
  recordScore(oneRoundScore.is1, roundtr);
  recordScore(oneRoundScore.is2, roundtr);
  recordScore(oneRoundScore.is3, roundtr);
  recordScore(oneRoundScore.is4, roundtr);
...
function resetPlayers(){
  var r =  confirm("DO YOU WANT RESET ALL THE DATA?");
  if (r==true){
    localStorage.clear();
...
resetform.addEventListener('submit', resetPlayers); 
